...
class ForensicFileViewer:
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(header_frame, text="🔍 File Viewer", bg="#2c3e50", fg="white", font=("맑은 고딕", 14, "bold"))
        title_label.pack(side=tk.LEFT, padx=20, pady=10)
        select_btn = tk.Button(header_frame, text="📁 폴더 선택", command=self.select_folder, bg="#3498db", fg="white", font=("맑은 고딕", 10), relief=tk.FLAT, padx=15, pady=5, cursor="hand2")
        ...
        tk.Label(search_frame, text="전체 검색:", bg="#f8f9fa", font=("맑은 고딕", 9)).pack(side=tk.LEFT, padx=(20, 5), pady=10)
        ...
        search_btn = tk.Button(search_frame, text="🔍 검색", command=self.search_files, bg="#28a745", fg="white", font=("맑은 고딕", 9), relief=tk.FLAT, padx=12, cursor="hand2")
        search_btn.pack(side=tk.LEFT, padx=5, pady=10)
        # new: separate search options
        tk.Checkbutton(search_frame, text="파일명 검색", variable=self.search_by_name, bg="#f8f9fa", font=("맑은 고딕", 9)).pack(side=tk.LEFT, padx=5)
        tk.Checkbutton(search_frame, text="내용 검색", variable=self.search_by_content, bg="#f8f9fa", font=("맑은 고딕", 9)).pack(side=tk.LEFT, padx=5)
        prev_btn = tk.Button(search_frame, text="◀ 이전", command=self.prev_search_result, bg="#6c757d", fg="white", font=("맑은 고딕", 9), relief=tk.FLAT, padx=10, cursor="hand2")
        prev_btn.pack(side=tk.LEFT, padx=2, pady=10)
        next_btn = tk.Button(search_frame, text="다음 ▶", command=self.next_search_result, bg="#6c757d", fg="white", font=("맑은 고딕", 9), relief=tk.FLAT, padx=10, cursor="hand2")
        next_btn.pack(side=tk.LEFT, padx=2, pady=10)
        self.search_info_label = tk.Label(search_frame, text="", bg="#f8f9fa", font=("맑은 고딕", 9))
        ...
        tree_label = tk.Label(left_frame, text="📁 폴더/파일 트리", bg="#495057", fg="white", font=("맑은 고딕", 10, "bold"), anchor="w", padx=10, pady=8)
        ...
        self.file_list_header = tk.Label(center_frame, text="📂 파일 목록", bg="#495057", fg="white", font=("맑은 고딕", 10, "bold"), anchor="w", padx=10, pady=8)
        ...
        self.file_listbox = tk.Listbox(center_frame, font=("맑은 고딕", 9), yscrollcommand=file_list_scroll_y.set, xscrollcommand=file_list_scroll_x.set)
        ...
        self.path_label = tk.Label(center_frame, text="", anchor="w", font=("맑은 고딕", 8))
        ...
        self.content_title_label = tk.Label(content_header, text="📄 파일 내용", bg="#6c757d", fg="white", font=("맑은 고딕", 9, "bold"))
        ...
        self.table_btn = tk.Button(view_mode_frame, text="표", command=lambda: self.change_view_mode("table"), bg="#495057", fg="white", font=("맑은 고딕", 8), relief=tk.FLAT, padx=8, cursor="hand2")
        self.table_btn.pack(side=tk.LEFT, padx=2)
        self.text_btn = tk.Button(view_mode_frame, text="텍스트", command=lambda: self.change_view_mode("text"), bg="#343a40", fg="white", font=("맑은 고딕", 8), relief=tk.FLAT, padx=8, cursor="hand2")
        self.text_btn.pack(side=tk.LEFT, padx=2)
        self.hex_btn = tk.Button(view_mode_frame, text="HEX", command=lambda: self.change_view_mode("hex"), bg="#343a40", fg="white", font=("맑은 고딕", 8), relief=tk.FLAT, padx=8, cursor="hand2")
        ...
        tk.Label(file_search_frame, text="파일 내 검색:", bg="#e9ecef", font=("맑은 고딕", 9)).pack(side=tk.LEFT, padx=(10, 5), pady=8)
        self.file_search_entry = tk.Entry(file_search_frame, font=("맑은 고딕", 9), width=30)
        self.file_search_entry.pack(side=tk.LEFT, padx=5, pady=8)
        self.file_search_entry.bind("<Return>", lambda e: self.search_in_file())
        file_search_btn = tk.Button(file_search_frame, text="찾기", command=self.search_in_file, bg="#17a2b8", fg="white", font=("맑은 고딕", 8), relief=tk.FLAT, padx=10, cursor="hand2")
        file_search_btn.pack(side=tk.LEFT, padx=5, pady=8)
        file_prev_btn = tk.Button(file_search_frame, text="◀", command=self.prev_file_search, bg="#6c757d", fg="white", font=("맑은 고딕", 8), relief=tk.FLAT, padx=8, cursor="hand2")
        file_prev_btn.pack(side=tk.LEFT, padx=2, pady=8)
        file_next_btn = tk.Button(file_search_frame, text="▶", command=self.next_file_search, bg="#6c757d", fg="white", font=("맑은 고딕", 8), relief=tk.FLAT, padx=8, cursor="hand2")
        file_next_btn.pack(side=tk.LEFT, padx=2, pady=8)
        self.file_search_info = tk.Label(file_search_frame, text="", bg="#e9ecef", font=("맑은 고딕", 9))
    ...
    def load_categories(self):
        ...
        try:
            ...
            for idx, category in enumerate(self.categories):
                btn = tk.Button(self.category_inner_frame, text=f"📂 {category}", command=lambda c=category: self.select_category(c), bg="white", font=("맑은 고딕", 9), relief=tk.RAISED, padx=15, pady=8, cursor="hand2")
    ...
    def insert_tree_node(self, parent, path):
        ...
        node = self.file_tree.insert(parent, "end", text=f"📁 {name}", values=(path,), tags=("dir",))
        try:
                        ...
                        try:
                            with os.scandir(path) as it:
                                ...
                                for entry in entries:
                                    ...
                                    if entry.is_dir():
                                        child = self.file_tree.insert(item, "end", text=f"📁 {entry.name}", values=(entry_path,), tags=("dir",))
                                        try:
                                        ...
                                        self.file_tree.insert(item, "end", text=f"📄 {entry.name}", values=(entry_path,), tags=("file",))
                        except Exception:
                            pass
    ...
    def on_file_select(self, event):
        ...
        if file_path:
            self.path_label.config(text=f"경로: {file_path}")
    ...
    def build_index_async(self):
        ...
        if not paths:
            return
        self.search_info_label.config(text="인덱스 생성 중...")
        self.root.update()
        def worker(path):
            try:
        ...
        with ThreadPoolExecutor(max_workers=8) as ex:
            ...
            for _ in as_completed(futures):
                pass
        self.search_info_label.config(text="인덱스 생성 완료")
    ...
    def search_files(self):
        ...
        if not do_name and not do_content:
            messagebox.showwarning("경고", "파일명 검색 또는 내용 검색 중 하나 이상 선택하세요.")
            return
        ...
        self.search_info_label.config(text=f"검색 결과: {len(self.search_results)}개 파일 (1/{len(self.search_results)})")
        self.add_search_category()
        self.show_search_result(0)
        messagebox.showinfo("검색 완료", f"{len(self.search_results)}개 파일에서 '{search_text}'를 찾았습니다.")

    def add_search_category(self):
        for widget in self.category_inner_frame.winfo_children():
            if isinstance(widget, tk.Button) and "🔍 검색 결과" in widget["text"]:
                widget.destroy()
        search_btn = tk.Button(self.category_inner_frame, text="🔍 검색 결과", command=self.show_search_results_tree, bg="#f39c12", fg="white", font=("맑은 고딕", 9), relief=tk.RAISED, padx=15, pady=8, cursor="hand2")
    ...
    def show_search_results_tree(self):
        ...
        for idx, result in enumerate(self.search_results, 1):
            ...
            self.file_tree.insert("", "end", text=f"📄 {rel_path}", values=(result['path'],))
        for widget in self.category_inner_frame.winfo_children():
            if isinstance(widget, tk.Button):
    ...
    def show_search_result(self, index):
        ...
        self.search_info_label.config(text=f"검색 결과: {len(self.search_results)}개 파일 ({index + 1}/{len(self.search_results)})")
    ...
    def prev_file_search(self):
        ...
        self.file_search_info.config(text=f"결과: {len(self.file_search_matches)}개 ({self.current_file_search_index + 1}/{len(self.file_search_matches)})")
